...
mod tests {
    ...
    fn test_poh_recorder_record_at_min_passes() {
        let ledger_path = get_tmp_ledger_path!();
        {
            ...
            let h1 = hash(b"hello world!");
            assert!(poh_recorder
                .record(bank1.slot(), h1, vec![tx.into()], false)
                .is_ok());
            assert_eq!(poh_recorder.tick_cache.len(), 0);
    ...
    fn test_poh_recorder_record_at_max_fails() {
        let ledger_path = get_tmp_ledger_path!();
        {
            ...
            let h1 = hash(b"hello world!");
            assert!(poh_recorder
                .record(bank.slot(), h1, vec![tx.into()], false)
                .is_err());
            for _ in 0..num_ticks_to_max {
    ...
    fn test_poh_recorder_record_transaction_index() {
        let ledger_path = get_tmp_ledger_path!();
        {
            ...
            let record_result = poh_recorder
                .record(bank.slot(), h1, vec![tx0.into(), tx1.into()], false)
                .unwrap()
                .unwrap();
            ...
            let record_result = poh_recorder
                .record(bank.slot(), h2, vec![tx.into()], false)
                .unwrap()
                .unwrap();
